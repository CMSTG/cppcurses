...
static std::map<char, chtype>
    graph_map; // This must be initialized after initscr()

static const std::array<short, 8>
    colour_map({COLOR_BLACK, COLOR_BLUE, COLOR_GREEN, COLOR_CYAN, COLOR_RED,
...
static inline void curs_visibility(void *win, int flag) {
...
static inline void curs_leaveok(void *win, bool flag) {
  leaveok((WINDOW *)win, flag);
}

static inline void curs_move(void *win, int y, int x) {
  wmove((WINDOW *)win, y, x);
}

static inline short curs_colorno(short bg, short fg) { return (bg << 3) | fg; }

static inline short curs_background(Window::colpair_t pair) {
  short colourno = rev_pair.at(pair);
  return (colourno >> 3) & 7;
}

static inline short curs_foreground(Window::colpair_t pair) {
  short colourno = rev_pair.at(pair);
  return colourno & 7;
}

static inline void curs_wmove(void *win, short y, short x) {
  wmove((WINDOW *)win, y, x);
}

static inline int curs_getch() {
...
void Window::init_maps(bool colour) {
  short pairno = 0;
...
Window::colpair_t Window::to_colour(Colour fg, Colour bg) {
...
static attr_t to_attrs(const char *attrstr) {
  attr_t attrs = 0;
  char ch;
...
static inline void curs_wattr_on(void *win, attr_t a) {
  wattr_on((WINDOW *)win, a, nullptr);
}

static inline void curs_wattron(void *win, Window::colpair_t colour) {
  wattron((WINDOW *)win, COLOR_PAIR(colour));
}

static inline void curs_wattr_off(void *win, attr_t a) {
  wattr_off((WINDOW *)win, a, nullptr);
}

static inline void curs_wattr_set(void *win, attr_t a, int pair) {
  wattrset((WINDOW *)win, a);
  wcolor_set((WINDOW *)win, pair, nullptr);
}

static inline void curs_waddch(void *win, chtype ch) {
  waddch((WINDOW *)win, ch);
}

static inline void curs_waddstr(void *win, const char *str) {
  waddstr((WINDOW *)win, str);
}

static inline void curs_wattr_get(void *win, Window::wattr_t &attrs,
                                  Window::colpair_t &pair) {
  attr_t a = 0;
  short p = 0;
...
static inline void curs_getyx(void *win, int &y, int &x) {
  getyx((WINDOW *)win, y, x);
}

static inline void curs_getparyx(void *win, int &y, int &x) {
  getparyx((WINDOW *)win, y, x);
}

static inline void curs_getbegyx(void *win, int &y, int &x) {
  getbegyx((WINDOW *)win, y, x);
}

static inline void curs_getmaxyx(void *win, int &y, int &x) {
...
Window &Window::addstr(const char *str) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window &Window::addstr(const std::string &str) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window &Window::addgrstr(const char *str) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
  char ch;
...
size_t Window::printf(const char *format, ...) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
  va_list ap;
  size_t n;

  va_start(ap, format);
  n = vw_printw(w, format, ap);
  va_end(ap);
  return n;
}

size_t Window::mvprintf(int y, int x, const char *format, ...) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
  va_list ap;
  size_t n;
...
Window &Window::move(int y, int x) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window &Window::attr_on(const char *attrs) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window &Window::attr_off(const char *attrs) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
  attr = wattr_t(to_attrs(attrs));
...
Window &Window::attr_set(const char *attrs, colpair_t pair) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
  attr = wattr_t(to_attrs(attrs));
...
Window &Window::colour(Colour fg, Colour bg) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window *Window::new_window(short y, short x, short nlines, short ncols) {
...
Window &Window::subwindow(short y, short x, short nlines, short ncols) {
  if (sub)
    delwin((WINDOW *)sub);
...
Window *Window::border_window(short y, short x, short nlines, short ncols) {
  Window *w = new Window(this, y, x, nlines, ncols);
...
Window::Window(Window *parent, short y, short x, short nlines, short ncols)
    : main(parent->main) {
...
Window &Window::move_window(short starty, short startx) {
  move_panel((PANEL *)panel, starty, startx);
  return *this;
}

Window &Window::get_yx(int &y, int &x) {
  curs_getyx((WINDOW *)win, y, x);
  return *this;
}

Window &Window::sub_yx(int &y, int &x) {
  if (sub)
...
Window &Window::sub_size(int &y, int &x) {
  WINDOW *w = sub ? (WINDOW *)sub : (WINDOW *)win;
...
Window &Window::orig(int &y, int &x) {
  curs_getbegyx((WINDOW *)win, y, x);
  return *this;
}

Window &Window::size(int &y, int &x) {
...
bool Window::is_supported(Key key) {
